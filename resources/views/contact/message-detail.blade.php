...
<div class="message-detail-container">
    <div class="detail-header">
        ...
        <div class="detail-meta">
            <div class="meta-item">
                ...
                <div class="meta-value">{{ $message->created_at->format('M d, Y') }}</div>
            </div>
            <div class="meta-item">
                <div class="meta-label">Time</div>
                <div class="meta-value">{{ $message->created_at->format('H:i A') }}</div>
            </div>
            @if ($message->status === 'replied')
                <div class="meta-item">
                    <div class="meta-label">Replied On</div>
                    <div class="meta-value">{{ $message->replied_at->format('M d, Y') }}</div>
                </div>
            @endif
        </div>
    </div>
    ...
    <div class="message-content">
        ...
        @if ($message->status === 'replied' && $message->admin_reply)
            <div class="reply-box">
                ...
                <p style="color: #999; font-size: 0.85rem; margin: 0;">
                    Replied on {{ $message->replied_at->format('M d, Y H:i A') }}
                </p>
        ...
        <div class="action-buttons">
            <a href="{{ route('contact.history') }}" class="btn btn-outline-primary">
